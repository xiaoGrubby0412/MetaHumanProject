...
public class GesturePanel : MonoBehaviour, IPointerUpHandler, IPointerDownHandler
{
    ...
    public enum UserInteractionOperationSet
    {
        MOVE,
        LOOK,
        ZOOM
    ...
    void LateUpdate()
    {
        ...
        if (Input.touchCount == 1) //单点触摸
        {
            ...
            //是否在拖动摇杆
            bool isFirstTouchLandedInJoystickArea =
                touch.position.x < (0.33f * Screen.width) && touch.phase == TouchPhase.Began;
            bool isTouchEnded = touch.phase == TouchPhase.Ended;
            if (isFirstTouchLandedInJoystickArea)
            {
                joyStickDraggingTouch = JoyStickDraggingTouch.Touch0;
            }
            ...
            //若没有拖动摇杆且单手操作，视作转向
            if (joyStickDraggingTouch == JoyStickDraggingTouch.NoTouch)
            {
                _Rotate(touch);
            } //单点触摸， 水平上下旋转
            else
        ...
        else if (Input.touchCount == 2) //多点触摸
        {
            ...
            //是否在拖动摇杆
            bool isFirstTouchLandedInJoystickArea0 =
                ((newTouch0.phase == TouchPhase.Began) || tempPointerDownFirstFrame) &&
                newTouch0.position.x < (0.33f * Screen.width);
            bool isFirstTouchLandedInJoystickArea1 =
                ((newTouch1.phase == TouchPhase.Began) || tempPointerDownFirstFrame) &&
                newTouch1.position.x < (0.33f * Screen.width);
            bool isTouchEnded = (newTouch0.phase == TouchPhase.Ended) && (newTouch1.phase == TouchPhase.Ended);
            if (isFirstTouchLandedInJoystickArea0)
            {
                joyStickDraggingTouch = JoyStickDraggingTouch.Touch0;
            }
            else if (isFirstTouchLandedInJoystickArea1)
            {
                joyStickDraggingTouch = JoyStickDraggingTouch.Touch1;
            }
            else if (isTouchEnded)
            {
                joyStickDraggingTouch = JoyStickDraggingTouch.NoTouch;
            ...
            else if (opSet.Contains(UserInteractionOperationSet.LOOK) &&
                     joyStickDraggingTouch == JoyStickDraggingTouch.Touch0)
            {
                _Rotate(newTouch1);
            }
            ...
            {
                Debug.LogWarning("[Zion] unKnown JoyStickDraggingTouch #: " + joyStickDraggingTouch.ToString());
            }
        }
        else
        {
            Debug.LogWarning("[Zion] unKnown touchCount #: " + Input.touchCount.ToString());
            return;
        }
    }
    ...
    void _Rotate(Touch touch)
    {
        ...
        playerThirdCamera.rotationX = playerThirdCamera.rotationX < 0
            ? Mathf.Clamp(playerThirdCamera.rotationX, -45, 0)
            : Mathf.Clamp(playerThirdCamera.rotationX, 0, 80); //将旋转值限制在极限值以内
    }
